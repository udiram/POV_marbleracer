...
def test_required_static_friction_matches_solid_sphere_result() -> None:
    config = SimulationConfig(angle_deg=30.0, length=20.0, height=13.0, static_friction_coeff=0.3)
...
def test_generated_course_contacts_multiple_obstacles() -> None:
    simulation = MarbleRampSimulation()
    ...
    for _ in range(3200):
        ...
        if sum(touched) >= max(2, len(touched) // 2):
            break
    assert sum(touched) >= max(2, len(touched) // 2)


def test_generated_course_exits_ramp_without_stalling() -> None:
    config = SimulationConfig()
    result = evaluate_course(config, max_time=20.0)
    assert result.exited_ramp
    assert not result.stalled
    assert sum(result.obstacle_hits) >= max(4, int(len(result.obstacle_hits) * 0.65))
    assert result.max_abs_lateral > 0.42
...
def test_obstacles_are_seated_on_ramp_surface() -> None:
    ...
    for obstacle in config.obstacles:
        ...
        assert isclose(center.z, expected_center_z, rel_tol=1e-6, abs_tol=1e-6)
